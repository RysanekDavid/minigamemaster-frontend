...
const GameApiService = {
  ...
  async getGameDefinitions(): Promise<GameDefinition[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch game definitions: ${response.statusText}`);
      }
  ...
  async generateGame(request: GenerateGameRequest): Promise<GameDefinition> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `AI generation failed: ${response.status} ${response.statusText} - ${errorData.message || "Unknown error"}`
        );
      }
  ...
  async startGame(gameId: string, options: any = {}): Promise<void> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: "Start game failed" }));
        throw new Error(`Failed to start game: ${errorData.message || response.statusText}`);
      }
    } catch (error) {
  ...
  async stopGame(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: "Stop game failed" }));
        throw new Error(`Failed to stop game: ${errorData.message || response.statusText}`);
      }
    } catch (error) {
  ...
  async getGameStatus(): Promise<GameStatus> {
    try {
      ...
      return { isActive: false, error: error instanceof Error ? error.message : "Unknown error" };
    }
  },
  ...
  async deleteGameDefinition(definitionId: string): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to delete game definition: ${response.statusText}`);
      }
  ...
  async updateGameConfig(gameId: string, config: any): Promise<void> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to update game configuration: ${response.statusText}`);
      }
    } catch (error) {
